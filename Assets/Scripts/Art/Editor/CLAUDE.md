...
- **TrimSheetAssets.cs** (296줄): 에셋 셋업. 메뉴 `Scavenger > Trim Sheet > Setup Stone Atlas Assets`
  ...
  - `EnsurePaletteMaterial()` / `EnsureTintedMaterial()` - 색만 다른 복사본.
    팔레트(배경)와 색상 키(필드 프리팹)가 같은 복사 로직을 쓴다
...
## 필드 프리팹 연동 (2026-07-26)

`Scavenger > Field Trim Sheet > Rebuild Field Prefabs`가 바닥 타일/행, 파밍 포인트,
아이템 오브젝트 프리팹을 트림시트 메시로 다시 굽는다
(구현: `Assets/Scripts/Game/Editor/FieldTrimSheetPrefabs.cs`, 어셈블리 Scavenger.Game.Editor).

- 프리팹 형태를 만드는 코드는 그대로 두고 `GreyboxBlockFactory.Source`만 잠시
  갈아끼운다 - 프리미티브 큐브 버전과 트림시트 버전의 형태 코드를 두 벌 두지 않기 위함.
  교체는 반드시 try/finally로 되돌린다
- 메시는 **(크기, 색) 조합마다 한 장**이고 이름이 곧 경로다
  (`Assets/Art/Meshes/TrimSheet/Field/Field_{x}x{y}x{z}_{RRGGBB}.asset`).
  색을 키에 넣는 이유는 같은 크기 배리에이션(바닥 타일 A/B/C)이 서로 다른 셀 배치를
  갖게 하기 위함이고, 시드도 색에서 뽑으므로 같은 색이면 같은 모양이 재현된다
  (`string.GetHashCode`는 실행마다 달라질 수 있어 FNV-1a로 직접 계산한다)
- 크기가 셀(0.5m)의 정수배가 아닌 얇은 축(바닥 두께 0.2m 등)은 타일 1장이 늘어난다.
  위/아래 면이 아니라 옆면이라 눈에 띄지 않는 선에서 허용한 것 - "타일 왜곡" 참조
- **직육면체 블록만 바뀐다.** 빌더가 큐브 생성기라 항아리 몸통 같은 원기둥은
  프리미티브로 남는다 (박스로 구우면 항아리가 상자가 된다)
