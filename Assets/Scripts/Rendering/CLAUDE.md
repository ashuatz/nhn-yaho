...
- LUT은 `(size-1)`로 굽고, 셰이더는 좌표를 텍셀 중심에 맞춘다:
  `uv = coord * (1 - texel) + 0.5 * texel`. 둘을 같이 써야 좌표 0/1이 첫/끝 텍셀
  중심에 정확히 앉는다. 텍셀 크기는 `_FogBlendParams.zw`로 전달된다.

## 원본 대비 수치 차이 (2026-07-25 재검증, Codex 교차확인)

원본과 대수적으로 검증한 결과. **감쇄 식 자체는 동일하고, 아래 3개만 다르다.**

1. **거리 입력이 근평면만큼 다르다.**
   원본은 `positionCS.z`를 `UNITY_Z_0_FAR_FROM_CLIPSPACE`로 통과시킨다.
   D3D reversed-Z에서 `clip.z = near·(far−eyeDepth)/(far−near)`이므로
   `clipZ_0Far = far·(eyeDepth−near)/(far−near) ≈ eyeDepth − near`.
   즉 원본의 거리는 `eyeDepth − 근평면`, 여기는 `eyeDepth`다.
   근평면 0.3 / `distanceRange 0~120` 기준 0.25% 차이라 눈에 보이지 않는다.
   URP 기본 포그와 정확히 맞추려면 `ComputeLinearEyeDepthFromWorld`에서
   `_ProjectionParams.y`를 빼면 된다. 의도적으로 빼지 않았다 -
   "카메라로부터의 거리(m)"가 아티스트에게 더 직관적이다.
2. **LUT 베이크 분모가 다르다** (원본 `/size`, 여기 `/(size-1)`).
   원본은 전 구간 `−1/256` 시프트가 있고 그라디언트 끝 키에 도달하지 못한다
   (UV 1에서 `gradient(0.996)`). 여기는 UV 1이 `gradient(1.0)`이다. 여기가 맞다.
3. **`density` 배율은 원본에 없다.** 기본값 1에서는 원본과 동일하고,
   1이 아니면 알파에 곱해져 달라진다. 의도한 추가 노브다.

## 알려진 모델 한계 (원본도 동일 - 고칠 대상이 아니라 알고 쓸 것)

- **색을 곱으로 합성한다** (`distColor * heightColor`, 알파 포함).
  그래서 "원거리=청색, 고공=주황색" 같은 **독립 색조를 표현할 수 없다**
  (`(0,0,1) * (1,0.5,0) = (0,0,0)` - 교차점이 검정이 된다).
  높이 램프는 거리 램프에 대한 감쇄/틴트 마스크로만 동작한다.
- 그 곱셈이 sRGB 인코딩 값끼리 일어난다 (선형 공간 곱이 아님).
  중간 회색에서 오차는 작지만 어두운 쪽에서 커진다. 원본과 맞추려고 유지했다.
- **높이 감쇄가 광선 적분이 아니라 픽셀의 월드 Y 한 점으로 결정된다.**
  낮은 안개층을 통과해 보이는 높은 물체는 "높은 곳"으로만 평가된다.
  물리적 높이 포그가 아니다.
- 기본 높이 그라디언트가 흰색/알파 1이라 **기본 상태에서 높이 효과가 0이다**
  (순수 거리 포그). 원본 기본값과 같지만 발견성이 나쁘다.
  높이 포그를 쓰려면 베이커에서 높이 램프의 알파를 아래=1, 위=0 방향으로 편집해야 한다.
