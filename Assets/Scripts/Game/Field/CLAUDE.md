...
- FarmingObject.cs: 아이템 오브젝트 (파밍 문서 3장). 오브젝트 스팟에 1대1로 놓이고
  상호작용 키(E) 홀드로 연다. 종류(Box/Jar) + 등급(FarmingPointGrade 공용) +
  상호작용 시간(등급별. DefaultInteractSeconds가 단일 소스) + 뚜껑/팝 자리는
  프리팹이 선언한다.
  진행 중에는 PlayerController를 Looting으로 두어 이동을 멈추고(ADR-0001),
  이동 입력 / 홀드 해제 / 범위 이탈 / 런 종료 / 구역 낙하면 즉시 중단한다.
  중단해도 오브젝트는 남는다. static Active(진행 중, 동시 1개) / PromptTarget(사거리 안) =
  HUD 게이지·버튼 참조.
  열리면 FarmingItemCatalog에서 아이템 1개를 뽑아 **LootSpot으로** 내놓는다 -
  가방 여유가 없으면 그 자리에 남는 규칙(문서 3.5)을 LootSpot이 이미 갖고 있다.
  아이템은 오브젝트가 아니라 **스팟(부모)의 자식**으로 붙인다 (열림 회전을 따라가지 않게)
- FieldSpawner.FarmingPoints.cs: 파밍 포인트 배치 (파밍 문서 2.4 / 2.5 + 3층 구조).
  ...
  PopulateObjectSpots = 스팟마다 아이템 오브젝트 1개 배치 (파밍 문서 3.2).
  파밍 포인트 등급 -> 오브젝트 등급 확률표(ObjectGradeChance)로 등급을 뽑고
  그 등급 프리팹 중 하나를 고른다 (그 등급 프리팹이 없으면 아무 것이나 - 스팟이
  통째로 비면 파밍할 것이 사라진다). 배치 전에 그레이박스 표식(SpotMarker)을 지운다.
  프리팹 미배선이면 코드로 상자 하나를 만든다 (BuildGreyboxFarmingObject)
...
| 아이템 오브젝트 | Assets/Prefabs/Field/Objects/ItemObject_Box\|Jar_Normal\|Rare\|Hero | 종류 2 x 등급 3 (파밍 문서 3.2) |

**트림시트 룩** (사용자 지적 2026-07-26: 스포너가 트림시트 에셋을 안 썼다):
위 프리팹들은 메뉴 **Scavenger > Field Trim Sheet > Rebuild Field Prefabs**로
...
- 아이템 오브젝트 데이터 테이블 (파밍 문서 3.2): 오브젝트 인덱스 / 아이템 그룹 인덱스 /
  아이템 등장 확률 컬럼이 아직 없어, 지금은 **오브젝트 등급 -> 아이템 등급 확률**
  한 단계로 줄여 두었다 (FarmingItemCatalog.Draw + ObjectGradeChance).
  상호작용 모션(진행/완료)도 캐릭터 애니메이션이 없어 뚜껑 젖힘 연출로 대체
