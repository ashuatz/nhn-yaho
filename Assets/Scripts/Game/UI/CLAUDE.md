...
- HudController.cs: HUD 패널 갱신 단일 지점. 실행 순서 -150 (입력 공급이
  PlayerController -100보다 먼저). 체력 바(HP, ADR-0008) - PlayerHealth.Normalized로
  Fill 폭 + HP 수치, Damaged 이벤트 구독으로 피격 붉은 화면 플래시(damageFlash 알파
  페이드, OnDisable 구독 해제). 참조 교체 시 재구독. 가치 합계 + 무게/적재 단계
  (M2-1, 이제 5단계 라벨),
