...
## 2026-07-26 (5) - 아래쪽 빈 공간 가리기 (저지대 바닥 + 하늘)

사용자 지시: "캐릭터 플레이하는 영역 아래쪽에 뻥 뚫린 부분을 안 보이게 하고 싶다.
1. 스카이박스 셋업 2. 아래쪽에 보이는 필드 셋업".

- **저지대 바닥** (팔레트 8, SegmentEnvironment.AddLowerGround): 보행면 -6m 아래에
  깔리는 지면. 존마다 36블록(가운데 열 + 좌우 4열 x z 4타일)이고 포그로 사라진다
  - 낙사 판정선(-4)보다 아래에 둔다. 위로 올리면 떨어진 플레이어가 착지한 것처럼 보인다
  - 콜라이더 없음 (배경 규칙) - 낙사는 그대로 성립
  - 타일을 x/z 1m씩 겹쳐 깐다. 딱 맞추면 스냅 보정/부동소수 오차로 이음새가 벌어진다
  - 드로우 배치 48 -> 56 (청크당 16 -> 18.7), 인스턴스 144 -> 273
- **하늘** (Editor/GreyboxSkySetup, 메뉴 Scavenger > Ensure Greybox Sky):
  카메라 지우기를 단색 -> 스카이박스로 바꾸고 그라디언트 하늘을 깐다
  (지평선 = 포그 톤, 위로 갈수록 깊어짐, 해 없음)
  - 이미지 생성은 못 했다: Unity MCP 이미지 제공자(fal / openrouter)에 **키가 없다**.
    아트 파노라마가 준비되면 같은 머티리얼의 셰이더만 Skybox/Panoramic으로 바꾸면 된다
  - LUT 포그가 하늘 픽셀을 skyDensity(0.85)만큼 덮으므로 하늘은 은은하게만 보인다.
    강한 그림보다 그라디언트가 맞는 이유
- 곁들여: 밝기 갈래에 중경/원경을 나눴다 (배경 배수 위에 곱해지는 미세 조정).
  밝기 창에 슬라이더 2개 추가 - 원경만 띄우거나 눌러 깊이감을 조절할 수 있다

---

## 2026-07-26 (4) - 밝기 조절 창 (GreyboxTheme)

사용자 지시: "이거 셋팅할 수 있는 에디터 윈도우좀".

- (3)에서 코드 상수에 직접 박은 배수를 **에셋으로 뺐다**. 코드에 두면 프리팹을 다시
  구울 때마다 조절값이 덮어써져 창을 만들 의미가 없다
- `Field/GreyboxTheme` (런타임 SO, Assets/Settings/GreyboxTheme.asset):
  전체 배수 + 갈래별 배수(필드 / 배경 / 캐릭터). 보정 규칙은 (3)과 같다
- 코드 색 상수는 **기준색**(색상 + 채도)으로 되돌리고, 색이 나가는 길목에서만 배수를 건다
  - 런타임: GreyboxPalette (Apply / GetTinted) 한 곳. 발광색은 제외
  - 에디터: GreyboxThemeAccess.Tint - 블록 팩토리 / 트림시트 틴트 / 배경 팔레트 /
    이름 있는 머티리얼(플레이어·그리드)
- 창: 메뉴 **Scavenger > Greybox Brightness** (Unity-Editor-Layout 규약).
  슬라이더 4개 + 기준색 -> 보정색 미리보기 19행(V 값 표시) + "적용"(프리팹·배경 재생성)
- 함정: Common.mat은 밝기 관리 대상에서 뺐다. **틴트의 기준(흰색)** 이라 여기에 배수를 걸면
  이 머티리얼을 복사해 만드는 트림시트 아틀라스까지 같이 어두워진다 (한 번 걸었다가 되돌림)
- 미배선 폴백은 배수 1이 아니라 기본값 1.7 - 0/1로 두면 배선을 잊었을 때
  화면만 어두워지고 원인을 찾기 어렵다

---

## 2026-07-26 (3) - 그레이박스 밝기 보정 (V x1.7)

사용자 지시: "전체적인 머티리얼 밝기가 너무 어두움. V값 다 올려".

- 규칙: **색상/채도는 그대로 두고 명도(V)만 x1.7**. V x1.7이 1을 넘으면 채도를 지키려
  V=1까지만 올린다 (채널을 개별로 자르면 색이 틀어진다)
- 코드 상수(정본)를 먼저 올렸다: SegmentEnvironment.Palette 8색 /
  FieldPrefabTemplates 타일·바닥·파밍포인트 / FieldSpawner 등급색 / Zone 바닥 /
  FarmingObjectTemplates 등급색 / GreyboxSceneSetup Common·플레이어 / 바닥 그리드.
  그 뒤 프리팹·배경 블록 세트를 다시 구워 생성 에셋에 반영
- 생성기가 색을 건드리지 않는 손 제작 머티리얼(Common / TrimSheet_Stone_01 /
  PlayerBody·Head / Env_Tile_01~07)은 1회 스크립트로 같은 규칙을 적용
- 배경 레이어 간 밝기 순서(근경 > 중경 > 원경)는 그대로다 - 깊이 구분의 근거라
  일괄 배수로 올려도 관계가 유지된다
- 참고: Common.mat / TrimSheet_Stone_01.mat의 베이스 컬러는 원래 흰색이었다.
  어두워 보인 원인은 베이스가 아니라 **틴트 색 상수**였다

---

## 2026-07-26 (2) - 에셋 이름 정리 + 배경 인스턴싱 트림시트화 + 가방 버리기

사용자 지시 3건: "머티리얼도 이름 해쉬마냥 만들어놓은거 있으면 싹다 사람이 읽기 쉽게.
어디에 사용되는건지", "gpu instancing 방식 스포너는 아직 트림시트 값으로 동작 안 함.
기존 사전 배치를 써야 한다면 존을 나아갈 때 계속 생성되도록 구상", "가방에서
드래그앤드랍으로 아이템 버릴 수 있어야 함 (다시 주울 수 있어야 함)".

### 1. 에셋 이름 = 용도

- 색상 해시 이름을 전부 없앴다. 이름은 "프리팹 + 부위"이고 메시는 뒤에 크기(cm)를 붙인다:
  `FarmingPoint_Top_Platform.mat` / `FarmingPoint_Top_Platform_500x20x900.asset`,
  `ItemObject_Box_Hero_Body.mat`, 배경은 `TrimSheetEnv_06_Midground.mat` /
  `Env_00_RubbleDark.mat` (팔레트 역할 이름 = SegmentEnvironment.PaletteNames)
- 이름은 GreyboxBlockFactory.Context(프리팹) + 블록 이름에서 나온다.
  템플릿마다 `using GreyboxBlockFactory.UseContext(prefabName)`으로 감싼다
- 메시 시드도 이름 해시라 다시 구워도 같은 모양이 재현된다 (색 해시 시드 폐기)
- 구 해시 에셋 74개는 참조 0을 확인하고 삭제

### 2. 배경 GPU 인스턴싱 트림시트화 (사전 배치로 후퇴하지 않음)

- 먼저 **재 봤다**: 배경 블록은 존당 74개, 셀 2m로 스냅하면 크기 종류가 존당 21개
  (최대 25). 즉 크기별 메시로 나눠도 인스턴싱 배치는 청크당 20개 남짓 -
  팔레트 8색만 쓰던 기존과 큰 차이가 없다. "크기별 메시 = 드로우콜 폭증"이라는
  이전 판단(2026-07-26 (1))이 과했다
- EnvironmentBlockSet(런타임 SO): 셀 스냅 크기 -> 구운 메시 + 팔레트 머티리얼.
  EnvironmentRenderer의 배치 키를 팔레트 -> (메시, 팔레트)로 바꿨다
- 스냅으로 커진 만큼 **윗면과 복도쪽 면을 유지**하도록 위치를 보정한다.
  위로 자라면 카메라측 지형이 발판을 가리고, 안쪽으로 자라면 통로를 침범한다
- 베이커는 실제 생성기를 8시드 x 6존 표본으로 돌려 나오는 크기만 굽는다 (60종).
  표본에 없던 크기는 가장 가까운 항목으로 대체
- 함정: 팔레트 머티리얼에 **GPU 인스턴싱이 꺼져 있으면** RenderMeshInstanced가
  매 프레임 예외를 던지고 배경이 통째로 사라진다 (실제로 겪었다).
  베이커가 켜 주고, 런타임은 1회 검사 후 큐브 폴백 + 경고

검증: 청크 3개 / 드로우 배치 48개(청크당 16) / 인스턴스 144개, 콘솔 에러 0.

### 3. 가방 드래그앤드롭 버리기

- 슬롯을 끌어 가방 밖에서 놓으면 발밑 앞 0.9m에 떨어진다. 가방 안에서 놓으면 취소
- RunInventory.TryDropOne / Restore: 무게·가치·**등급**을 그대로 들고 나갔다 들어온다.
  합성해 둔 등급이 버렸다 줍는 것만으로 사라지지 않는다.
  BankedCounts도 되돌린다 (버리고 줍기를 반복해 창고 개수를 부풀리는 구멍 차단)
- LootSpot: 버린 직후에는 잠겨 있고 플레이어가 수집 반경을 한 번 벗어나야 열린다 -
  발밑에서 즉시 되빨리면 버리기가 성립하지 않는다
- 월드 배치는 FieldSpawner.DropBagItem (생성 경계 유지). 바닥 행에 부착돼 함께 낙하

검증 (플레이 모드): 3개 -> 1개 버리기 -> 가치 30->20, 월드에 배치, 되돌아와 주우면
30 복구 / 합성한 등급2를 버렸다 주워도 등급2 유지 (가치 60, 무게 1.0),
BankedCounts 5 유지.

---

## 2026-07-26 (1) - 대시보드 UI 전환 + 아이템 오브젝트 + 필드 프리팹 트림시트화

사용자 지시 3건: "대시보드에 있는 데이터도 적절히 UI 구성해서 놔둘 것 (대시보드는 접기)",
"파밍 포인트에 인터렉션 가능한 파밍 객체 배치도 있어야 함 (더미 프리팹 구성해서 넣기)",
"필드 스포너가 트림시트로 만든 에셋을 사용하지 않음 (개선 필요)".

### 1. 진단 대시보드 (IMGUI -> UI Toolkit)

- RunDebugDashboard.cs 삭제, RunDashboardView.cs 신규. HUD와 **같은 UIDocument**
  (HudDocument 프리팹)에 Dashboard.uxml을 CloneTree로 얹는다 - 패널 설정/폰트를
  HUD와 공유하려는 것 (별도 문서를 두면 폰트를 또 넣어야 하고, 폰트 누락은
  글자가 아예 안 나오는 함정이다)
- 기본 접힘. 접힌 상태에서도 헤더에 요약(깊이 / 런 상태 / 붕괴 거리)이 보인다.
  헤더 클릭 = 접기, F1 = 표시/숨김. 접힘 표시는 텍스트 [+] / [-] (이모지 금지 규약)
- 섹션 4종 RUN / FIELD / PLAYER / LOOT. 진행도는 막대, 붕괴 거리는 12m 이내면 경고 색.
  갱신은 0.1초 간격 (매 프레임 문자열 생성 회피)
- 기존 RunSystems 프리팹에서 컴포넌트를 제거했다 (프리팹 YAML 직접 편집).
  Assets/Prefabs/Dummy.prefab(백업 덤프)에는 참조가 남아 열면 missing script가 뜬다

### 2. 아이템 오브젝트 (파밍 문서 3장)

- FarmingObject.cs: E 홀드로 진행 -> 열림 -> 파밍 아이템 1개를 LootSpot으로 내놓는다.
  중단 조건은 이동 입력 / 홀드 해제 / 범위 이탈 / 런 종료 / 구역 낙하 (문서 3.3 (4)).
  획득 제한(무게·슬롯 초과 시 그 자리에 남음, 문서 3.5)은 LootSpot이 이미 갖고 있어
  그대로 재사용했다 - 파밍 전용 획득 경로를 새로 만들지 않았다
- FarmingItemCatalog.cs: 고가치 4종(60/150/400/1000), 합성 N = 3 (문서 4.2).
  아이템 그룹 테이블이 아직 없어 **오브젝트 등급 -> 아이템 등급 확률** 한 단계로 축소
- 더미 프리팹 6종 (상자/항아리 x 일반/희귀/영웅):
  Assets/Prefabs/Field/Objects/ItemObject_*. 영웅만 포인트 라이트를 단다
  (전부 달면 존마다 광원이 수십 개)
- 배치: PopulateObjectSpots가 스팟마다 1개. 파밍 포인트 등급이 오브젝트 등급 확률을
  정한다. 난수는 전부 RunManager.Rng, 오브젝트에는 rng.Next()로 뽑은 시드를 넘긴다
  (런타임 직접 소비 금지 규약)
- HUD: 하단 중앙 게이지(이미 UXML에 있었지만 미사용이던 loot-gauge)를 상호작용
  진행도에 연결하고, 상호작용 버튼 노출 조건에 아이템 오브젝트를 추가

검증 (플레이 모드, MCP): 존 4개 파밍 포인트에 스팟 8개 / 오브젝트 8개 배치,
영웅 항아리 상호작용 3초 -> 열림 -> 도금 왕관(t4) 팝 -> 접근 시 자동 흡수,
인벤토리 가치 1000 / 무게 9.0 반영. 콘솔 에러 0.

### 3. 필드 프리팹 트림시트화

- 스포너는 프리팹을 인스턴스화할 뿐이라 아트 교체 지점은 프리팹이다.
  런타임 코드를 건드리지 않고 프리팹의 메시/머티리얼만 트림시트 산출물로 바꿨다
- GreyboxBlockFactory: 프리팹 템플릿의 블록 생성 훅. 기본은 프리미티브 큐브,
  트림시트 경로가 Source를 잠시 갈아끼운다 (형태 코드는 한 벌만 유지)
- 메뉴 Scavenger > Field Trim Sheet > Rebuild Field Prefabs.
  결과: 블록 37개 / 정점 6,744개 / 새로 구운 메시 27종 (셀 0.5m)
- 배경 인스턴싱은 제외 - 배경 블록은 스케일이 임의값이라 크기별 메시를 쓰면
