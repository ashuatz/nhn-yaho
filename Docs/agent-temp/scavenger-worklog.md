...
- Setup Greybox Scene 재실행 필요 (세션 중 플레이 모드라 불가) - HudCanvas
  프리팹 GUID가 바뀌어 기존 씬의 인스턴스가 끊겼다. 플레이 종료 후
  Scavenger > Setup Greybox Scene 1회
- EditMode 테스트도 플레이 종료 후 1회 (dotnet 3 프로젝트는 0 에러 확인)
