...
- 조작 (ADR-0007): WASD/화살표 홀드 = 2D 벡터 이동 (대각 허용), 좌하단 가상
  조이스틱(드래그 = 아날로그), E 홀드 루팅, W/E 선택지, 클릭=라운드 재개, F1 대시보드
- 압박: 바닥 붕괴(CollapseFront) + 땅꺼짐/밀기 트랩 + 폭탄. 위험 범위는 바닥 셀
  점멸(DangerGrid). 카메라 쉐이크 = 피격/근접 피드백. 낙사 있음. 점프 없음
...
6. Test Runner (EditMode) 실행 - 42개 통과 여부 (35개까지는 실기 확인, 신규 7개는
   컴파일만 확인 - 플레이 모드 중이라 미실행)
